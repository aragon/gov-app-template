...
import { type IInputContainerProps, InputContainer } from "@aragon/ods";
...
export const TextAreaRichText: React.FC<ITextAreaRichTextProps> = (props) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "outline-none p-4 ![overflow-wrap:anywhere] prose prose-neutral min-h-[160px] h-full max-w-none leading-normal",
        role: "textbox",
    ...
    onUpdate: ({ editor }) => {
      const value = editor.getText() !== "" ? editor.storage.markdown.getMarkdown() : " ";
      onChange?.(value);
    },
  });
